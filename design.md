...
**다크 모드는 현재 미적용**(코드에 `.dark` 클래스 셀렉터는 있으나 토글 UI 없음). 위 표는 "적용하게 될 때"의 시작점이며, §9 후속 우선순위에 배치.
...
현재 다크 모드는 **미적용**(`.dark` CSS 셀렉터는 존재하나 토글 UI·시스템 감지 로직 없음). 적용 시:
...
4. **Pretendard 폰트 전환** — §3.1 세 가지 방법 중 선택, 폰트 파일 도입은 승인 필요.
5. **다크 모드 완성** — §2.3의 `(추정)` 값 재검증 + 토글 UI 구현.
