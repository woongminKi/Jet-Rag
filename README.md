...
**상태**: v0.1 MVP 개발 중 (W5 Day 4 완료, 2026-05-02 기준 / 6주 일정 중 5주 완료, 잔여 W6 = 사용자 자료 누적 + 정량 KPI)
...
| 평가 | golden 평가셋 v0.2 (W5 Day 4) — 자연어 10 + 키워드 5 + 메타 5 + DOCX 5 |
...
| `work-log/2026-05-02 W5 종합 정리 + W6 진입 핸드오프.md` | **최신 진입 핸드오프** — W6 작업자가 가장 먼저 읽을 문서 |
...
## 현재 가용 기능 (2026-05-02 W5 Day 4 마감)

### 백엔드 엔드포인트
- `POST /documents` — 멀티파트 업로드 (**PDF/HWP/HWPX/DOCX**/PPTX/이미지/TXT/MD/URL, 최대 50MB), 매직바이트 검증 (filetype + DNS rebinding 방어), SHA-256 dedup, 7스테이지 파이프라인 비동기 시작
...
- `GET /search?q=` — **하이브리드 검색** (PGroonga sparse + pgvector dense + RRF k=60) + doc 그룹화 + relevance + matched_chunks + meta filter (doc_type/date_from/date_to) + `query_parsed` 응답
- `GET /stats` — 시스템 통계 + **search_slo** (p50/p95/avg + cache_hit_rate + fallback breakdown, ring buffer 500건)
...
- **S2 검색** (`/search?q=`) — 결과 카드 + 매칭 청크 하이라이트 + relevance + 503 에러 처리 (Server Component, Next.js 16)
- **S6 인제스트** (`/ingest`) — 드래그앤드롭 + 8스테이지 실시간 진행 (1.5s 폴링)
...
| 출처 일치율 | ≥ 0.95 | **0.95 (golden 19/20)** | ✅ |
| P95 검색 응답 (cache hit) | ≤ 500ms (자체) / ≤ 3s (절대) | **159ms** | ✅ |
| chunk_filter e2e | — | HWPX 18~22% 자동 마킹 | ✅ |
| 단위 테스트 회귀 | 0 | 155/155 PASS | ✅ |
