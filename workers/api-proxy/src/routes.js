 ...
 *
 * 이관 중에는 **점진 전환의 스위치**였다 — 한 줄을 더하면 그 경로만 Supabase 로 넘어가고,
 * 지우면 즉시 Railway 로 되돌아왔다. Phase 2~5 를 그 방식으로 진행했다.
 *
 * **2026-09-08 이관 완료.** `LEGACY_ORIGIN` 을 비워서 이제 표에 없는 경로는 Railway 가 아니라
 * **404** 다. 줄을 지우면 그 경로가 죽는다 — 되돌릴 백엔드가 없다.
 *
 ...
 *
 * 세 번째 칸을 주면 **그 메서드만** 넘긴다. 나머지 메서드는 `LEGACY_ORIGIN` 으로 갔었고,
 * 이관이 끝난 지금은 404 다. 같은 경로에 메서드별로 다른 라우트가 붙어 있고 그중 일부만
 * 옮긴 단계에서 필요했다 — 예: `/documents` 의 POST(업로드)만 먼저 Edge 로 넘기던 때.
 *
