...
|---|---:|---:|---:|
| `idx_chunks_dense` | 237~265MB | **56MB**(SK 완료 후; 036 직후 25MB) | shared_buffers 224MB 의 절반, **110MB** |
| `chunks` 총 크기 | 506MB | **144MB** | — |
| 벡터 수(dense_vec NOT NULL) | 14,281(필터 4,612 포함) | **22,077**(미필터 전체) | halfvec 기준 약 **40,000** 개에서 110MB |
...
4. 임베딩은 **매분 cron(`* * * * *`) + 태스크당 64개** 그대로 둔다(약 64벡터/분). 더 빠르게 돌리지 않는다.
   ...
   **마이그 037 부터 태스크는 `chunks_set_dense_vec` RPC 32행씩(fsync 2회)으로 쓴다.** 그 전 코드로 되돌리지 않는다.
   실측(2026-09-16 07:50~08:39): 같은 매분 cron 으로 41태스크 실패 0, 검색 0.5~3.3s — 단건 UPDATE 때는 3시간에 5회 실패·6~61s.
