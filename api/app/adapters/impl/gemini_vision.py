...
기획서 §10.4 의 단일 호출 JSON 계약을 그대로 구현. 호출자는 정규화된 bytes 를
넘기는 책임 (`ImageParser` 가 다운스케일·EXIF transpose 담당, QA 검수 C-2). 본 구현은:

- google-genai SDK `inline_data` 로 이미지 1장 전달
- response_mime_type=application/json + `_PROMPT` 로 5필드 보장
...
S4-A D1 — table_caption / figure_caption 2필드 추가 (≤ 60자, 없으면 null).
표/그림 검색 보강용. caption (전체 요약) 과 별개로 동시 비-null 가능. cache key 는
`_VISION_PROMPT_VERSION` v1 → v2 bump 로 invalidate (vision_cache.py).
"""
...
#   · 그 외 → null
# - 한국어 출력 강제
# S4-A D1 — table_caption / figure_caption 추가 (≤ 60자, 없으면 null).
#   · 페이지에 표 N개여도 "첫 표 1개" 만 추출 (D1 단순화)
#   · structured.표.headers/rows 와 병행 — table_caption 은 검색용 한국어 제목
_PROMPT = """\
...
class GeminiVisionCaptioner:
    ...
    def _parse(
        ...
        *,
        ...
        # S4-A D1 — table_caption / figure_caption (≤ 60자 권고 — 모델측 강제 X).
        # 부재 / null / 비-str 모두 None 으로 graceful. 빈 문자열도 None 처리해
        # 검색 보조 인덱스 (S4-B) 가 의미 없는 row 를 만들지 않도록.
