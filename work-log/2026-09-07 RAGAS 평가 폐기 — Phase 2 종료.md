...
## 52. 골든셋 — **§51.5 의 진단이 틀렸다** (`ce6b090`)

### 52.1 정정

§51.5 에 "골든셋이 낡았다" 고 적었다. **틀렸다.** 내가 본 건 `golden_batch_smoke.py` 에
인라인으로 박힌 **레거시 v0.1 20 건**이었고, `evals/` 의 진짜 골든셋은 멀쩡하다.

| 세트 | 행 | 고유 doc | 현재 DB 생존 |
|---|---:|---:|---|
| `evals/golden_v2.csv` | 132 | 9 | **9 — 전부 있음** |
| `evals/golden_v1.csv` | 157 | 13 | 9 (**40 행이 죽은 문서 4 개 참조**) |
| v0.1 인라인 (스모크) | 20 | 5 | **0 — 전부 죽음** |

원인은 단순하다 — **`evals/` 를 안 열어보고** 스모크 스크립트 하나로 "골든셋" 을
판단했다. `evals/` 에는 `build_golden_v2.py` · `compare_golden_edge.py` ·
`auto_goldenset.py` 까지 있는 평가 체계가 통째로 있었다.

> §50.1 에서 "Railway cron 이 돌고 있지 않다" 를 정정한 것과 같은 종류다.
> **한 파일만 보고 시스템 상태를 단정했다.**

### 52.2 고친 것

`golden_batch_smoke.py` 가 CSV 를 읽는다(기본 `golden_v2.csv`, `--goldenset` 로 교체).
인라인 세트는 지웠다 — 죽은 기대값을 유지하는 것보다 진짜 세트를 쓰는 게 맞다.

**죽은 문서를 가리키는 행을 조용히 버리지 않고 센다.** `golden_v1` 은 러너가 40 행을
말없이 걸러 123 행만 돌아 왔다 — 그래서 아무도 몰랐다.

| | top-1 | top-3 | 지연 |
|---|---|---|---|
| 전 | 0/20 (0.0%) | 0/20 | — |
| 후 | **103/120 (85.8%)** | **110/120 (91.7%)** | p50 244ms · p95 564ms |

전의 0% 는 검색 품질이 아니라 **기대값이 죽어 있던 것**이다. 그 상태로는 회귀를 못 잡는다.

### 52.3 MMR 을 제대로 쟀다 (§51.3 보강)

`evals/compare_golden_edge.py` 가 골든셋을 Railway·Edge 에 나란히 돌린다 —
**MMR 변경은 이걸로 쟀어야 했다.** 123 행 전체:

| 모드 | 응답 불일치 | Recall@10 | MRR | nDCG@10 |
|---|---|---|---|---|
| doc-scope (limit 50) | **0 건** | 0.4616 / 0.4616 | 0.3766 / 0.3766 | 0.4112 / 0.4112 |
| multi-doc (limit 10) | **1 건** (G-A-053) | 0.2944 / 0.2944 | 0.3262 / 0.3262 | 0.2969 / 0.2969 |

**MMR 은 123 행 중 1 행의 2 위 이하 순서만 바꾸고 지표는 소수점 4 자리까지 같다.**
top-1 이 안 바뀌므로 MRR 이 그대로인 게 설명된다. §51.3 의 "골든 20 건 발화 0 건" 은
맞았지만 표본이 20 건이었고, 이제 123 건으로 확인했다.

`compare_golden_edge.py` 에도 `INTENTIONAL_DIVERGENCE` 를 넣었다 — 사유를 매번 찍고,
**적어 놓고 실제로는 일치하면 실패**시킨다.

### 52.4 남은 것 (사용자 결정)

`golden_v1.csv` 의 **40 행**이 사라진 문서 4 개를 가리킨다. 그 문서를 다시 올릴지,
행을 뺄지는 결정 사항이다. 기준선(2026-09-04 R@10 0.4722)이 v1 조건에서 나왔으므로
v1 을 손대면 기준선과의 비교가 끊긴다 — 그래서 손대지 않고 **보고만 한다.**

### 52.5 이번 세션 자[尺] 오판 — 일곱·여덟 번째

- `--goldenset` 이 이미 있는 줄 알고 넘어갔는데, 실은 **주석 문자열**에 걸린 것이었다.
  구조가 아니라 부분 문자열로 확인한 탓이다.
- `JETRAG_API_BASE_URL` vs `JET_RAG_API_BASE` — 같은 것을 두 이름으로 부르고 있어
  20 건 전부 err 이 났다(§51.6 에서 이미 고쳤다).
...
|---|---|
| **A** | secret 4개 설정 → 이메일 + 결제 컷오버 — Railway 종료의 남은 차단 요인 전부 |
| **B** | `golden_v1` 의 죽은 40행 처리 (§52.4) — 문서 재업로드 vs 행 삭제, 결정 필요 |
| **C** | PPTX Vision 보강 (`_vision_ocr_largest_picture`, §41.6) |
| **D** | chunk 조각 c2 `synonym_inject` (200줄) |

---

## 53. PPTX Vision 보강 이관 (`bc629a7`). **재인제스트가 문서를 지울 뻔했다**

§41.6 에서 "미이관" 으로 남겨 둔 `_vision_ocr_largest_picture` 를 옮겼다.

### 53.1 왜 급했나 — 지금 재인제스트하면 이 문서가 비워진다

실측: 코퍼스의 `브랜딩_스튜디오앤드오어.pptx` 는 **11 장 전부 텍스트 0 자 + 그림 다수**다.
`extractPptx` 는 텍스트 0 슬라이드를 버리므로 Edge 만으로는 **0 청크**가 된다.

DB 의 실제 값은 **5 청크**이고 `section_title` 이 전부 `p.N (Vision OCR)` — Railway 가
이 경로로 넣은 것이다. 안 옮긴 채 재인제스트하면 문서 내용이 통째로 사라진다.

> §47(`/documents/url` 폐기)에서 "사용 0 건이면 옮길 게 없다" 를 적용했는데, 여기는
> 정반대다. **쓰이고 있는 경로**였고, 그 사실은 DB 를 열어 봐야 알 수 있었다.

### 53.2 설계 판단을 실측으로 했다

| 질문 | 실측 | 결정 |
|---|---|---|
| CPU 2s 안에 5 장이 들어가나 | 상위 5 장 정규화 합계 **432ms**(9KB·1048KB·15KB·107KB·9KB) | `scan` 처럼 창을 나누지 않고 **한 태스크** |
| 텍스트 0 슬라이드를 어떻게 되살리나 | `extractPptx` 가 버려서 못 살린다 | `extractPptxSlides` 를 새로 노출하고 `extractPptx` 를 그 위에 다시 얹음. 동작 불변 확인 — xml 파서 대조 **686 건 / 0** |

### 53.3 옮긴 규칙

- 상한 5 는 **시도** 기준 — 성공만 세면 실패할 때마다 다음 슬라이드로 넘어간다
- 텍스트 0 → rerouting(`pptx_rerouting`), 1~49 → augment(`pptx_augment`, 기존 **뒤에** 붙임)
- 가장 큰 그림 1 장. **동점이면 앞의 것** — Python `max()` 가 그렇다(`>=` 로 쓰면 갈린다)
- `<a:blip r:embed>` → `_rels` → `ppt/media/...` 경로 해석 (§41.6 의 미이관 사유였던 것)
- quota 감지 시 남은 슬라이드는 **호출조차 안 한다**

### 53.4 같이 채운 구멍 2 건

**① quota 감지가 Edge 에 아예 없었다.** `vision_usage_log.quota_exhausted` 컬럼은 있는데
아무도 안 채웠다 — §45 의 `image_parser` 이식에서 빠뜨린 것이다. `quota_detect.ts` 로
원본 3 단계 판정(클래스명 → `429` 속성 → 메시지)을 옮기고 실패 경로에 연결했다.

**② `parseImage` 에 `sourceType` 인자가 없었다.** 전부 `image` 로 기록되고 있었다.
원본은 호출처가 `pptx_rerouting`/`pptx_augment` 를 명시한다.

### 53.5 대조 — **자를 한 번 고쳤다**

`verify_pptx_vision_parity.py` — **15 건 / 0**.

처음엔 가짜 **캡셔너**를 주입했는데 5 건이 전부 불일치로 나왔다. 원인: 캡셔너는
**정규화된** 바이트를 보므로 이미지 재인코딩 차이(§45.3, 별도로 163 건 대조함)가 섞인
것이었다. 구조·슬라이드 선택·제목은 처음부터 다 맞았는데 해시만 달랐다.

`ImageParser.parse` 자리를 갈아끼워 **원본 blob** 의 sha256 을 비교하도록 바꾸니
선택·순서·상한만 남았다. 그러려고 `parseImageFn` 주입 지점을 하나 열었다.

그러고도 1 건이 남았는데 또 **`"Error: "` 접두사**였다 — §51 과 같은 패턴의 **네 번째**
자리다. 이 문자열은 문서 `warnings` 로 저장돼 사용자에게 보인다. `pyStrError` 로 고쳤다.

음성 대조: 섹션 텍스트 변조를 검출.

### 53.6 라이브 E2E (실제 파일 3.15MB)

```
extract → chunk → tag_summarize → load → embed → doc_embed → dedup  전부 succeeded
청크 5 / dense_vec 5/5          ← Railway 결과와 같은 수
태그 6개: 스튜디오 · 핵심 가치 · 서비스 · 사진 · 고객 · 촬영
vision_usage_log: source_type='pptx_rerouting' 5건 · 합계 $0.0073
```
...
|---|---|
| **A** | secret 4개 설정 → 이메일 + 결제 컷오버 — Railway 종료의 남은 차단 요인 전부 |
| **B** | chunk 조각 c2 `synonym_inject` (200줄) — 마지막 기능 잔여 |
| **C** | `golden_v1` 죽은 40행 처리 (§52.4) — 결정 필요 |

---

## 54. `synonym_inject` 이관 (`6738bae`). **기능 이관의 마지막 조각**

chunk 조각 c2. `chunk_records.ts` 가 `notPortedInjector` 로 **던지며** 자리를 잡아 뒀던
곳을 실제 구현으로 채웠다. (조용히 건너뛰지 않고 던지게 해 둔 게 이번에 도움이 됐다 —
어디를 채워야 하는지가 코드에 남아 있었다.)

### 54.1 왜 옮겨야 했나 — 이미 데이터에 있다
...
켜진 적이 있고 결과가 남아 있다. 안 옮긴 채 재인제스트하면 그 199 개의 sparse(PGroonga)
매칭이 사라진다 — §53(PPTX)과 같은 종류의 손실이다.

읽기 쪽(`stripSynonymMarker`)은 **이미 이관돼 있었다**(`search/snippet.ts` ·
`answer/chunks.ts`). 그래서 마커가 사용자에게 보이진 않았다. 빈 건 **쓰기 쪽**뿐이었다.

### 54.2 사전은 손으로 옮기지 않았다

`_DOMAIN_SYNONYMS` 36 entry 를 스크립트로 뽑아 넣었다. **순서가 결과를 바꾸기** 때문에
`Map` 으로 담았다 — 후보가 사전 순서로 쌓이다 **cap 5 에서 잘리므로**, 순서가 곧
"어떤 후보가 살아남는가" 다. 객체 리터럴은 정수형 키를 앞으로 당기는 규칙이 있어
순서를 언어 규칙에 맡기지 않았다.

사전 전용 대조: **키 순서 36 개 일치 + 양방향 조회 111 개 토큰 전량 일치.**

### 54.3 옮긴 규칙 — 갈리기 쉬운 것들

- **양방향 조회**(키로도 값으로도), 자기 자신 제외, 순서 보존 dedupe **2 회**
- **cap 5** — 사전 순서 의존
- **이미 본문에 있는 후보 제외** — **부분 문자열** 기준이다(형태소 아님)
- 후보의 `[`·`]` 제거 — 안 하면 마커 구조가 깨져 `strip` 이 반만 떼고 나머지가 노출된다
- `.strip()` 은 Python 공백 집합(`pyStrip`) — JS `trim()` 과 다르다
- **코드펜스 벗기기**: Python `split("```", 2)` 는 **최대 2 회 분할**이라 JS `split` 의
  limit 과 의미가 다르다. 인덱스로 직접 잘랐다
- LLM 후보는 **doc 당 정확히 1 회**, chunk 루프 **밖**에서. 실패는 전부 빈 배열
- `raw_text` 는 Edge 가 창으로 쪼개 저장하므로 `loadRawText` 로 되붙여 넘긴다(앞 3000 자만 씀)

### 54.4 검증

| 항목 | 결과 |
|---|---|
| `verify_synonym_inject_parity.py` | **43 건 / 불일치 0** |
| cap 5 가 실제로 발동한 케이스 | **6 건** — 순서 의존 경로가 진짜로 걸린다 |
| 음성 대조 | 후보 배열 변조 검출 |
| **배선 테스트** | ENV on → 마커+metadata / off → 완전 무변경 / 사전 미적중 → 무변경 (3 건 추가) |
| 라이브 E2E (`law_sample2.pdf`) | 청크 9 · dense_vec 9/9 · ENV off 라 무변경 — 회귀 0 |
| CI | edge 243 → **246** · proxy 20 · py unittest · 대조 14/14 |

**배선까지 테스트한 이유**: 모듈이 맞아도 `toChunkRecords` 경유가 안 물리면 소용없다.
`notPortedInjector` 가 던지던 자리를 실제로 지나가는지 확인해야 한다.

### 54.5 남은 것 (사용자 판단)

`JETRAG_SYNONYM_INJECTION_ENABLED` 가 Edge 에 미설정(기본 false)이라 지금은 경로가
안 돈다. 켜는 건 **인제스트 동작 변경**이라 사용자 판단이다 — 켜면 재인제스트 시
199 개 마커가 복원된다.

### 54.6 기능 이관 현황

앱 라우트 이관은 §47 에서 닫혔고, **기능(인제스트 파이프라인) 잔여도 이걸로 0** 이다.

| 항목 | 상태 |
|---|---|
| 앱 라우트 33 개 | Edge 28 · Railway 5(FastAPI 자체 문서) — §47 |
| 인제스트 9 단계 | 전부 이관 — §40 |
| 지원 포맷 | pdf · hwp · hwpx · docx · pptx(+Vision) · image — §53 |
| 입력 채널 | 업로드 · 이메일 · 공유 타깃 — §42·§45 |
| 결제·cron | 코드 완료, secret·마이그 적용 대기 — §46·§50 |
| chunk 조각 c2 | **이 커밋** |
...
## 56. `LEGACY_ORIGIN` 을 비웠다 — **Railway 의존 종료** (`5a86ba7`, 2026-09-08)

### 56.1 변경 전후를 같은 명령으로 쟀다

| 대상 | 전 | 후 |
|---|---|---|
| 앱 라우트 11 건 (`/health` · `/search` · `/documents*` · `/stats*` · `/auth/me` · `/me/plan` · `/ingest/email` · `/billing/run`) | 200/401 | **전부 동일** |
| `/` · `/docs` · `/openapi.json` · `/redoc` | 307/200/200/200 | **전부 404** |

사라진 건 FastAPI 자체 문서뿐이다. 옮길 대상이 아니라 **Railway 와 함께 사라지는 게
맞는 것들**이다.

실사용 확인: `/search "휴관일"` total=5 정상 · 인제스트 사슬 E2E 정상
(`.txt` 는 §43 대로 graceful skip).

### 56.2 리허설이 이제 **실제 상태**를 읽는다

`rehearse_railway_shutdown.ts` 의 "지금" 값이 하드코딩이라, 실제로 끈 뒤에도
**"끄면 5 건이 달라진다"** 를 계속 보여 줬다 — 지나간 예측을 현재 상태로 읽게 된다.
`wrangler.toml` 에서 `LEGACY_ORIGIN` 을 읽도록 고쳤다.

```
라우트 33개 · LEGACY_ORIGIN = (비어 있음 — 이미 껐다)
  그대로 도는 것 33건
  끄면 달라지는 것 **0건**
  저장소 ↔ 배포 드리프트 0
```
...
`wrangler.toml` 의 `LEGACY_ORIGIN` 에 원래 값을 넣고 재배포하면 즉시 복구된다.
그 값과 사전 확인 명령을 파일 주석에 적어 뒀다.

### 56.4 **이건 Railway 를 끈 게 아니다**

프록시가 부르지 않을 뿐 **Railway 서비스는 계속 돌고 요금도 나온다.** 며칠 지켜본 뒤
사용자가 서비스를 삭제해야 실제로 끝난다.

### 56.5 Railway 종료 체크리스트 — **전부 완료**

```
1. secret 3개 설정                       ✅
2. 프록시 배포 → 드리프트 0                ✅
3. billing cron 마이그 029 적용            ✅
4. monitor-search-slo API base            ✅
5. 실제 메일 E2E                          ✅
6. LEGACY_ORIGIN 비우기                   ✅  ← 이 커밋
─────────────────────────────────────────────
7. Railway 서비스 삭제                     ⬜  사용자 · 관찰 기간 후
```
...
| 항목 | 성격 |
|---|---|
| Railway 서비스 삭제 | 관찰 기간 후 사용자 판단 (§56.4) |
| `JETRAG_KAKAOPAY_SECRET_KEY` | 카카오페이 심사 대기 — 발급되면 넣으면 결제 활성 |
| `JETRAG_SYNONYM_INJECTION_ENABLED` | 켤지 결정 (§54.5) — 켜면 재인제스트 시 199개 마커 복원 |
| `golden_v1` 죽은 40행 | 문서 재업로드 vs 행 삭제 (§52.4) |
| Fernet 키 일치 확인 | 암호문 0건이라 지금은 확인 불가 (§55.7) |
