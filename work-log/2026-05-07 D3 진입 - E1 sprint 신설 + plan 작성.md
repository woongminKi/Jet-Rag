...
> **D3 진입 — 8 commit push** (`a5cfc2a`→`f1c87b4`). baseline 회복 + E1 sprint plan 신설 (ETA "3분" vs 실측 6~7분 격상) + S0 D4 자동 POST 제거 (비용 누수 fix) + S0 D4 P2 follow-up 3건 + S1 D1 잔여 (`auto_goldenset.py` v2 + 30 신규 테스트) + **E2 fixture 인프라 1·2·3·4차 ship** (`assets/public/` 8건 git 추적 + 5단계 우선순위 5개 형식 (PDF/HWPX/HWP/PPTX/DOCX) 자동 인식). 단위 테스트 460 → **496 통과 / skipped 0 / 회귀 0**. assets/ 14건 중 12건 자동 회귀 (남은 2건은 HWP OLE2 아님). E1 진단·구현은 PDF 보유 다른 컴퓨터에서 진입.
...
> 2026-05-07 D3 진입 — 8 commit push (`a5cfc2a`→`f1c87b4`). baseline 회복 + E1 sprint plan 본문 ship + S0 D4 자동 POST 제거 (+ P2 follow-up 3건) + S1 D1 잔여 (`auto_goldenset.py` v2 + 30 테스트) + **E2 1·2·3·4차 ship** (`assets/public/` 8건 git 추적, 5단계 우선순위 5개 형식 (PDF/HWPX/HWP/PPTX/DOCX) 자동 인식, `test_hwp_heading.py`/실 fixture 테스트 클래스 신규). 단위 테스트 460→496 / skipped 0 / 회귀 0. assets/ 14건 중 12건 자동 회귀. E1 진단·구현은 PDF 보유 다른 컴퓨터에서 진입.
...
# 사용자 PC 기대 (E2 4차 ship 후): Ran 496 tests in ~13s, OK  (PDF 5 + HWPX 2 + HWP 1 + sonata + PPTX 1 + DOCX 2 자동 진입, skipped=0)
# 다른 컴퓨터·CI 기대: Ran 496 tests in ~13s, OK (skipped 4~6)  (assets/ 직속 부재 → sonata·PPTX·DOCX 자동 skip)
```
...
**시나리오별 동작** (E2 4차 ship 후)

```text
시나리오 1: 자료가 <repo>/assets/ 직속에 있는 컴퓨터 (사용자 PC 패턴)
  → 2순위 자동 진입, ENV 0
  → 단위 테스트 496 통과 / skipped 0
  → assets/ 14건 중 12건 자동 회귀 (PDF 5 + HWPX 2 + HWP 1 + sonata + PPTX 1 + DOCX 2 + alpha)
  → 남은 2건 (HWP OLE2 아님: cosmetic_law_sample, law sample2) 은 처리 불가
...
- **E2 follow-up (남은 항목)**:
  ...
  - HWP 의 OLE2 아닌 자료 (`cosmetic_law_sample.hwp`, `law sample2.hwp`) 처리 가이드 — Hwp5Parser 가 인제스트 단계에서 거부하므로 fixture 자체 부적합 (private 유지). 향후 hwp_parser.py 가 OLE2 아닌 컨테이너도 지원하도록 확장 시 fixture 추가 가능. **남은 미진입 자료 = assets/ 14건 중 2건만**
