...
### 4.3 S1.5 — **100% 마감** / S2 — **D1 ship + D4-pre 회귀 측정 통과 (~25%)** / S3·S4·S5 — **0% 미진입**
...
- **S2** (vision 선별 본 ship) — D1 ship (2026-05-08, `extract.py` + `incremental.py` 양쪽 hook + ENV 회복 토글) + **D4-pre 회귀 측정 통과** (2026-05-08, mock + DB + baseline R@10 결합 옵션 C — 회귀 위험 1건이지만 baseline R@10=1.000 으로 실 영향 ≈ 0, **S2 D2 진입 가능 결정**). 다음: D2 (page cap + cost budget 병행) → D3 (운영 모드 UI) → D4 (옵션 A 본격 회귀 측정) → D5 (효과 보고).
...
| **단위 테스트 baseline** | **612 통과 / skipped 1 / 회귀 0** (시작 시 460, S1.5 D1 PoC 11 + S1.5 D3 v2 +24 누적) → 2026-05-08 S2 D1 누적 시 **619 / skipped 1 / 회귀 0** (+7) |
...
| **Master plan §6 S2 진척률** (2026-05-08) | **D1 본 ship + D4-pre 회귀 측정 통과** (~25%) — D1 (extract + incremental hook + ENV 회복 토글, 단위 테스트 +7) + D4-pre (mock + DB + baseline R@10 결합 옵션 C, paid call 0, **회귀 위험 1건이지만 baseline R@10=1.000 으로 실 영향 ≈ 0 → S2 D2 진입 가능 결정**). 다음: D2 cap 정합 / D3 운영 모드 UI / D4 옵션 A 본격 회귀 측정 / D5 효과 측정 |
...
| 1 (다음) | **S2 D2 — page cap + cost budget 정합 검증** | 0.5~1일 | S0 D4 budget_guard 와 needs_vision OR rule 의 통합 작용 검증 + 사용자 의도 cap 정책 결정 (사용자 확인 필요) |
...
| 3 (다음) | **S2 D3 — 운영 모드 UI 토글** (`/admin/vision-need-score`) | 1~1.5일 | 운영자가 임계 조정 / 페이지 cap 조정. Web 변경 필요 |
