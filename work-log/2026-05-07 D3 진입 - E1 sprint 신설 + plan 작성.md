...
| **Master plan §6 S2 진척률** (2026-05-08) | **D1 본 ship 완료** (extract + incremental hook + ENV 회복 토글 + 단위 테스트 +7). 다음: D2 cap 정합 / D3 운영 모드 UI / D4 회귀 측정 / D5 효과 측정 |
...
| 1 (다음) | **S2 D4 — 본 PC 5 PDF 회귀 측정** (priority ↑) | 0.5일 | needs_vision skip 적용 후 인제스트 + 골든셋 recall 5/6 ±5pp 안 유지 검증. -5pp+ 시 stop + S1.5 v3 우선 |
| 2 (다음) | **S2 D2 — page cap + cost budget 정합 검증** | 0.5~1일 | S0 D4 budget_guard 와 needs_vision OR rule 의 통합 작용 검증 + 사용자 의도 cap 정책 결정 (사용자 확인 필요) |
