...
| ship | **S1.5 D1 — vision_need_score PoC** (senior-developer 구현, 별도 work-log [`2026-05-07 S1.5 D1 vision_need_score PoC ship.md`](./2026-05-07%20S1.5%20D1%20vision_need_score%20PoC%20ship.md)) | `api/app/services/vision_need_score.py` (~140 LOC) + `api/scripts/poc_vision_need_score.py` (~190 LOC) + 단위 테스트 11건 신규. master plan §6 S1.5 default (entity regex / table-like 0.5 / density 1e-3) hand-tuned 그대로. 운영 코드 (`extract.py`) 미참조 — D2/D3 측정 후 통합 결정. 1차 PoC: 5 PDF / 115 페이지 / **needs_vision 54.8%** (entity 0 / table_like 19 / low_density 51). 단위 테스트 577 → **588 통과 회귀 0**. commit `a6fedc2` |
| ship | **S1.5 D2 — 분포 분석 + 임계 조정안 + vision_diagram 정확도** (senior-developer 직접 ship, 별도 work-log [`2026-05-07 S1.5 D2 vision_need_score 분포 분석.md`](./2026-05-07%20S1.5%20D2%20vision_need_score%20%EB%B6%84%ED%8F%AC%20%EB%B6%84%EC%84%9D.md)) | (A) **`api/scripts/analyze_vision_need_score.py` 신규 (~310 LOC, stdlib 만 — pandas/numpy 회피)** + **`evals/results/vision_need_score_d2_analysis.csv` 신규** (per-page detail + composite score + 임계별 boolean, gitignore 됨). (B) **분포 분석** — 5 PDF / 115 페이지 / composite score (등가중 합산) **p50=0.20 / p90=0.39 / max=0.46** → D1 default 임계 0.5 invalid. histogram 10 bins 모두 [0.0~0.5] 안. (C) **신호 간 Pearson 상관 (3×3)** 모두 |r|<0.11 → **3 신호 독립 (중복 0)**. entity 는 raw 0 분산이라 상관 정의 불가. (D) **임계 시뮬레이션** — score 0.30 → needs 33% / 0.50 → 0% / 0.70 → 0%. 신호별: density<1e-3 → 44.3% / table≥0.5 → 16.5%. target 비율 역산 (needs 30%↔score 0.31 / 50%↔0.20 / 70%↔0.11). (E) **vision_diagram + table_lookup recall (golden v1, N=6 매칭)** — D1 OR rule 5/6 = **83.3%** (데이터센터 p.40 표 1건 false negative — table_score 0.00). composite score 임계별 recall (0.3/0.5/0.7 = 33%/0%/0%). NFC/NFD 정규화 fix 포함 (macOS 파일명 NFD vs 골든셋 NFC). (F) **D1 관찰 3건 정정** — (a) entity 8 후보 패턴 본문 hit 2/115 → **deprecated 권고**. (b) sonata catalog 29 pages 중 density<1e-3 = 20/29 (69%) → **1e-3 임계 유지** (카탈로그 신호 보호). (c) **table 0.5 → 0.3 권고** + table 휴리스틱 v2 (한국어 cell span 분리 fallback) D3 본 ship. (G) **D3 결정 사항 7건 정리** — OR rule 유지 / density 1e-3 / table 0.3 / table v2 / entity deprecate / 6 신호 중 미측정 3종 (image_area_ratio / text_quality / caption_existence) 추가 / 11 docs 측정 후 본 결정. 단위 테스트 588 / skipped 1 / **회귀 0** (analysis only, stdlib 만, 의존성 0) |
| ship | **S1.5 D3 — 임계·가중 정정 + needs_vision OR rule + S2 진입 결정** (senior-developer 직접 ship, 별도 work-log [`2026-05-07 S1.5 D3 ship — 임계·가중 정정 + S2 진입 결정.md`](./2026-05-07%20S1.5%20D3%20ship%20%E2%80%94%20%EC%9E%84%EA%B3%84%C2%B7%EA%B0%80%EC%A4%91%20%EC%A0%95%EC%A0%95%20+%20S2%20%EC%A7%84%EC%9E%85%20%EA%B2%B0%EC%A0%95.md)) | (A) **`vision_score.py` v2** (D1 ~140 → D3 ~270 LOC) — `needs_vision()` / `needs_vision_breakdown()` API 신규, OR rule (entity 제외 5 신호), `compute_score()` composite 보존 (hybrid mode 진입 여지). (B) **D2 결정 7건 모두 close** — table 0.5→0.3 / table v2 single-span fallback (다중 공백·탭 분리) / entity deprecated (`DEFAULT_WEIGHTS["entity_density"]=0.0` + OR rule 제외) / 미측정 3 신호 추가 (image_area_ratio ≥0.30 / text_quality ≤0.40 / caption_score ≥0.20) / composite 가중치 재분배 (entity 0.20→0, density_inverse 0.10→0.30). (C) **`scripts/poc_vision_need_score.py` v2** — D1 9 컬럼 호환 + D3 9 컬럼 추가 (image_area_ratio / text_quality / caption_score / composite_score / trigger_* 5종). D2 분석 스크립트는 D1 컬럼만 읽어 호환성 보존. (D) **본 PC 5 PDF 재측정** — needs_vision_or = 74/115 = **64.3%** (D1 의 54.8% → +9.5%p, image_area 32 + caption 7 + table 0.3 회수 +7). sonata catalog 69.0% → **89.7%** (image_area 신호 회수). composite score max 0.46 → **0.51** (entity 회수분 density 가산). (E) **골든셋 vision_diagram + table_lookup recall = 5/6 = 83.3% 유지** — 데이터센터 p.40 false negative 동일 (PyMuPDF 가 표를 cell 별 line 분리 → table v2 fallback 의 single-span 다중공백 추정 효과 X, multi-line table 휴리스틱 v3 sprint 후보). (F) **단위 테스트 35건** (D1 11 → +24 — TableV2FallbackTest 3 / NeedsVisionOrRuleTest 8 / EntityDeprecatedTest 3 / ImageAreaSignalTest 3 / TextQualitySignalTest 3 / CaptionSignalTest 2 / CompositeScoreTest 2 / 갱신 1). 자체 trust-but-verify 1건 — `any(dict.items())` truthy tuple 버그 catch + `.values()` fix. **588 → 612 / skipped 1 / 회귀 0**. (G) **S2 D1 본 ship 진입 prerequisite 충족** — `extract.py` page 별 OR rule hook 으로 약 36% 페이지 vision call 회피 가설 (115 pages → 74 pages). 운영 코드 격리 유지 (extract.py / pymupdf_parser.py / chunk.py 미참조) |
| ship | **S2 D1 — vision_need_score 운영 hook + 페이지 선별 작동** (2026-05-08, senior-developer 직접 ship, 별도 work-log [`2026-05-08 S2 D1 vision_need_score 운영 hook ship.md`](./2026-05-08%20S2%20D1%20vision_need_score%20%EC%9A%B4%EC%98%81%20hook%20ship.md)) | (A) **`extract._enrich_pdf_with_vision` + `incremental._vision_pages_with_sweep` 페이지 루프 hook** — `_page_needs_vision()` 헬퍼 양쪽 module-level. needs_vision False 페이지는 ImageParser 호출 회피 + sweep retry 대상 X (논리 일관 — 첫 sweep skip = 모든 sweep skip). (B) **graceful fallback** — `_score_page_for_vision` raise 시 needs_vision=True (보수적). cache lookup 흐름 보존 (cost 0). (C) **ENV 회복 토글** `JETRAG_VISION_NEED_SCORE_ENABLED` (default true) — `Settings.vision_need_score_enabled: bool` + `_parse_bool()` 헬퍼 신규. invalid ENV 는 default fallback. false 시 S1.5 이전 동작 100% 보존. (D) **메트릭 = log 1줄** (warnings X, flags X) — `vision_enrich: file=X processed=Y called=Z skipped_need_score=N pages=[..]` 운영 진단용. (E) **단위 테스트 +7** — `test_extract_pdf_vision_enrich.py:TestVisionNeedScoreHook` 4건 (skip / sweep retry X / ENV false / 점수 raise graceful) + `test_incremental_vision.py` 신규 파일 3건 (skip / sweep / ENV false). **612 → 619 / skipped 1 / 회귀 0**. (F) **사이드 이펙트 0** — DB 마이그 0 / 외부 API 0 / 새 패키지 0 / web 변경 0 (S2 D3 에서 운영 모드 UI). S0 D4/D5 budget cap + S0 D2-B vision_cache 통합 상태 유지 |
...
| ship | **S2 D4-pre — 회귀 측정 (needs_vision skip 의 골든셋 영향)** (2026-05-08, senior-developer 직접 ship, 별도 work-log [`2026-05-08 S2 D4-pre 회귀 측정.md`](./2026-05-08%20S2%20D4-pre%20%ED%9A%8C%EA%B7%80%20%EC%B8%A1%EC%A0%95.md)) | (A) **옵션 C (Hybrid) 채택** — auto mode 자율 판단. mock 시뮬레이션 + DB 정답 chunk page cross-check + baseline retrieval R@10 통합. paid call 0 / cap $1.00 안 통과 ($0.00 실측). (B) **`evals/run_s2_d4_pre_regression.py` 신규 (~526 LOC)** — D3 측정 CSV (`vision_need_score_d3.csv`) 의 needs_vision 결정 + 골든셋 v1 의 vision_diagram + table_lookup 9건 cross-check + `chunks` 테이블 page 컬럼 lookup + `search()` 직접 호출 baseline R@10. 측정 결과물 (`s2_d4_pre_regression.md` + `_hints.csv` + `_chunks.csv` + `_retrieval.csv`) gitignore 됨. (C) **본 PC 5 PDF / 115 pages → 41 skip (35.7%, 가설 36% 정확 매칭)** — 데이터센터 PDF 87.8% skip (41 → 5 called) / sonata 10.3% / 포트폴리오 0% / 보건의료 0% / law sample3 50.0%. vision call 절감 효과 = text-heavy 정부 자료에 집중 발현 정합. (D) **회귀 위험 1건 식별** — G-A-008 (데이터센터 p.40 table_lookup) 의 hint page = needs_vision=False (모든 OR rule 미통과). DB 정답 chunk 374 의 page 도 40 으로 일치. (E) **결정적 통찰 — baseline R@10 = 1.000** 으로 정답 chunk 374 가 top-3 retrieve. 즉 정답 chunk 가 vision-derived 가 아닌 일반 PDF text extraction 산출 → vision skip 영향 ≈ 0. (F) **권고: ✅ S2 D2 진입 가능** — 표면 회귀 위험은 1건이지만 실 회귀 위험 0. S1.5 v3 (multi-line table 휴리스틱) 후속 후보 ★ 유지 (정확도 향상 차원, 우선순위 낮음). (G) **운영 코드 변경 0** — extract.py / incremental.py / vision_need_score.py 미변경. DB 마이그 0 / 외부 API 0 / 새 패키지 0 / web 변경 0. **단위 테스트 619 / skipped 1 / 회귀 0** 유지 |
...
### 4.3 S1.5 — **100% 마감** / S2 — **D1 + D4-pre + D2 ship (~40%)** / S3·S4·S5 — **0% 미진입**

- **S1.5** (PoC 3일) — D1/D2/D3 모두 ship 완료 (2026-05-07). `services/vision_need_score.py` + 단위 테스트 35건 + composite score (hybrid 진입 여지 보존) + OR rule (운영 채택).
- **S2** (vision 선별 본 ship) — D1 ship (2026-05-08, `extract.py` + `incremental.py` 양쪽 hook + ENV 회복 토글) + D4-pre 회귀 측정 통과 (2026-05-08, mock + DB + baseline R@10) + **D2 ship (2026-05-08, page cap + cost budget 병행)** — `check_doc_page_cap()` 신규 (in-memory 카운터, DB 미접근), ENV `JETRAG_VISION_PAGE_CAP_PER_DOC` (default 50, 0=무한), `BudgetScope` 'page_cap' 추가, `_mark_page_cap_exceeded_flag()` (cost cap flag 와 직교), needs_vision skip 페이지는 카운터 증가 X (cap 도달 지연 정합). 단위 테스트 619→**632 (+13) / 회귀 0**. 다음: D3 (운영 모드 UI, Q-S2-1 사용자 결정 의존) → D4 (옵션 A 본격 회귀 측정) → D5 (효과 보고).
- **S3·S4·S5** — `api/app/services/` 에 `intent_router.py`, `meta_filter.py`, `decomposition.py`, `reranker.py` 없음. 본격 진입 신호 없음. S2 마감 후 순차.
...
| 항목 | 값 |
|---|---|
| **오늘 commit 누적** | **23 commits** (`a5cfc2a`→`c143102` 누적 push + S1.5 D3 commit 1건 push 대기) |
| **단위 테스트 baseline** | **612 통과 / skipped 1 / 회귀 0** (시작 시 460) → 2026-05-08 S2 D1 +7 → S2 D2 +13 누적 시 **632 / skipped 1 / 회귀 0** |
...
| **Master plan §6 S1.5 진척률** | **D1 PoC + D2 분석 + D3 본 ship 완료** (운영 통합은 S2 D1 에서 hook — `extract.py` 의 page 별 OR rule 분기) |
| **Master plan §6 S2 진척률** (2026-05-08) | **D1 + D4-pre + D2 ship (~40%)** — D1 (extract + incremental hook + ENV 회복 토글, 단위 테스트 +7) + D4-pre (mock + DB + baseline R@10) + **D2 (page cap + cost budget 병행, in-memory 카운터, ENV `JETRAG_VISION_PAGE_CAP_PER_DOC` default 50, needs_vision skip 페이지 카운터 증가 X, 단위 테스트 +13)**. 다음: D3 운영 모드 UI (Q-S2-1 사용자 결정 의존) / D4 옵션 A 본격 회귀 측정 / D5 효과 측정 |
...
# 3) 회귀 baseline 확인
cd ../api && uv run python -m unittest discover tests
# 기대: Ran 612 tests, OK (skipped 0~3 — 다른 컴퓨터 시나리오 따라 가변)
...
| # | 후보 | 작업량 | 의존성 / 진입 신호 |
|---|---|---|---|
| 1 | ~~S2 D1 — Vision 선별 본 ship~~ **2026-05-08 ship 완료 (✅)** — `extract._enrich_pdf_with_vision` + `incremental._vision_pages_with_sweep` 양쪽 hook + ENV 회복 토글 + 단위 테스트 +7 | done | done |
| 1.5 | ~~S2 D4-pre — 회귀 측정 (needs_vision skip 의 골든셋 영향)~~ **2026-05-08 ship 완료 (✅)** — 옵션 C (Hybrid) mock + DB + baseline R@10. **회귀 위험 1건 식별 (G-A-008 데이터센터 p.40), baseline R@10=1.000 으로 실 영향 ≈ 0 → S2 D2 진입 가능 결정** | done | done |
| 1.6 | ~~S2 D2 — page cap + cost budget 병행~~ **2026-05-08 ship 완료 (✅)** — `check_doc_page_cap()` 신규 (in-memory 카운터, DB 미접근), ENV `JETRAG_VISION_PAGE_CAP_PER_DOC` (default 50, 0=무한), `BudgetScope` 'page_cap' 추가, `_mark_page_cap_exceeded_flag()` (cost cap flag 와 직교), needs_vision skip 페이지 카운터 증가 X (cap 도달 지연 정합). 단위 테스트 619→**632 (+13) / 회귀 0** | done | done |
| 1-alt | S2 D4 — 옵션 A 본격 회귀 측정 (실 reingest 5 PDF + 골든셋 v1 전수 R@10 baseline vs after) | 0.5일 | D4-pre 의 mock 한계 (불명 5건) 보완. 비용 ≈ $0.32 (5 PDF × 22p × 64% × $0.0045/p). cache hit 시 절감 |
| 1 (다음) | **S2 D3 — 운영 모드 UI 토글** (`/admin/vision-need-score`) | 1~1.5일 | 운영자가 임계 조정 / 페이지 cap 조정. **Q-S2-1 (운영 모드 default) 사용자 결정 필수**. mode → page cap 매핑 (빠른=10, 기본=50, 정밀=0). web 변경 동반 |
| 2 | **E1 진단 + 1차 ship 잔여 (A2/A3/A4/A5)** | 1.5~3일 | PDF 보유 다른 컴퓨터 reingest (E1 plan §3 SQL S1~S5) → §10 채움 → senior-developer 의뢰. **사용자 ETA 보고 직접 해소** (가치 ↑↑) |
| 3 | **S1.5 v3 — multi-line table 휴리스틱 (데이터센터 p.40 회복)** | 1일 | 인접 line bbox y 좌표 + 폭 패턴 / PyMuPDF drawings ruling line 추출. 골든셋 recall 5/6 → 6/6 회복. S2 D1 의 회귀 측정에서 -5pp+ 일 때 priority 격상 |
| 4 | **S1.5 D3 보강 — 다른 컴퓨터 11 docs 재측정** | 0.5일 | 사용자 PC 11 docs (PDF 5 + HWP 1 + HWPX 2 + DOCX 2 + PPTX 1) — `cd api && uv run python scripts/poc_vision_need_score.py --pdf-dir <11 docs path>` → D3 v2 CSV 합산 → 임계 정정 (image_area 0.30 / caption 0.20 가설 검증). 비-PDF 4 포맷은 PoC 자체 out-of-scope (D3 도입 검토). vision_diagram 매칭 6→9 row 가능 |
| 4 | **S1.1 (가칭) — reliable 100+ sample 모델 회귀 재측정** | 0.5일 | KST 02~05시 503 회복 시간대 재실행 + RAGAS judge timeout 600s 늘림 + sample 100+ (~$0.5~1.0). Δ 신뢰도 확보 후 lite 채택/hybrid/flash 유지 결정 |
| 5 | **S0 D3~D5 — budget 초기값 / cost cap / 24h 대시보드** | 1.5일 | 1주 vision_usage_log 누적 데이터 의존. 진입 신호: ≥2026-05-14 |
| 6 | **S2 — Vision 선별 + budget 본 ship** | 1주 | S0 D3~D5 ship 후 진입 (budget 의존). master plan §6 S2 |
| 7 | **RAGAS 점수 개선 (베타 테스트 후 판단)** | 0.5~1일 | 베타 사용자 5~10명 누적 후 진입. 6 옵션 우선순위 (LLM judge / query expansion / Reranker / doc RRF / decomposition / caption). 메모리 `project_ragas_score_beta.md` |
