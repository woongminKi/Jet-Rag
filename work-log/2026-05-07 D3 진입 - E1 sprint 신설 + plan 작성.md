...
| **단위 테스트 baseline** | **612 통과 / skipped 1 / 회귀 0** (시작 시 460, S1.5 D1 PoC 11 + S1.5 D3 v2 +24 누적) |
...
| # | 후보 | 작업량 | 의존성 / 진입 신호 |
|---|---|---|---|
| 1 | **S2 D1 — Vision 선별 본 ship (`extract.py` page 별 OR rule hook)** | 1~1.5일 | S1.5 D3 ship 완료 (✅) — 즉시 진입 가능. `_enrich_pdf_with_vision` 에서 D3 OR rule 적용 → 약 36% 페이지 vision call 회피 가설 (115→74). **회귀 측정**: 골든셋 v1 vision_diagram + table_lookup 응답 정확도 ±5pp 안 유지. -5pp+ 시 즉시 stop + S1.5 v3 (multi-line table) 우선 |
