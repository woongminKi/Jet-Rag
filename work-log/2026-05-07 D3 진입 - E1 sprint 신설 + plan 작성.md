...
| 항목 | 값 |
|---|---|
| **오늘 commit 누적** | **22 commits** (`a5cfc2a`→`a6fedc2` 누적 push + S1.5 D2 commit 1건 push 대기) |
| **단위 테스트 baseline** | **588 통과 / skipped 1 / 회귀 0** (시작 시 460, S1.5 D1 PoC 11 신규) |
...
| **Master plan §6 S1.5 진척률** | **D1 PoC + D2 분석 ship** (D3 본 ship 대기 — 사용자 PC 11 docs 측정 의존) |
...
| 2 | **S1.5 D2 보강 — 다른 컴퓨터 11 docs PoC 재실행** | 0.5일 | 사용자 PC 11 docs (PDF 5 + HWP 1 + HWPX 2 + DOCX 2 + PPTX 1) — `cd api && uv run python scripts/poc_vision_need_score.py --pdf-dir <11 docs path>` → CSV 합산 → `analyze_vision_need_score.py` 재수행 → D2 분석 보강. 비-PDF 4 포맷은 PoC 자체 out-of-scope (D3 도입 검토). vision_diagram 매칭 6→9 row 가능 (G-U-016 직제규정 / G-A-021 sample-report / G-A-011 브랜딩 진입) |
| 3 | **S1.5 D3 본 ship** | 1.5~2일 | (S1.5 D2 보강 후 진입) — OR rule 유지 / table 임계 0.3 + 휴리스틱 v2 / entity deprecate / image_area_ratio · text_quality · caption_existence 3 신호 추가 계측 / 운영 통합 (`extract.py` 페이지별 vision 분기). 사용자 결정 7건 (D2 §5) 선결 |
