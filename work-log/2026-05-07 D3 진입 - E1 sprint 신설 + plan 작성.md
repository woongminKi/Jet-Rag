...
> **D3 진입 — 8 commit push** (`a5cfc2a`→`f1c87b4`) + **S1 D3·D4 ship 진입** (실 query 로그 시각화 + answer_feedback 통합 분석 대시보드). baseline 회복 + E1 sprint plan 신설 (ETA "3분" vs 실측 6~7분 격상) + S0 D4 자동 POST 제거 (비용 누수 fix) + S0 D4 P2 follow-up 3건 + S1 D1 잔여 (`auto_goldenset.py` v2 + 30 신규 테스트) + **E2 fixture 인프라 1·2·3·4·5차 ship** (`assets/public/` 8건 git 추적 + 5단계 우선순위 5개 형식 (PDF/HWPX/HWP/PPTX/DOCX) 자동 인식 + **negative path 회귀 (D 그룹) + frontend 가독성 보강**) + **E1 1차 ship 일부 즉시 진입** (인제스트 진행 표시 3대 증상 일괄 fix — ETA 정적 / progress bar 1칸 / stage 카운터 1/8 vs 9) + **S1 D2 ship — 자동 골든셋 100+ 확장 + v1 통합 + retrieval fallback chain v1 우선** + **S1 D3 ship — 실 query 로그 시각화 대시보드** + **S1 D4 ship — answer_feedback 통합 분석 대시보드** (`/admin/feedback` 페이지 신규, `GET /admin/feedback/stats` endpoint 신규, 일별 sparkline + 코멘트 룰 분류 4 카테고리 + 최근 10건 코멘트 — LLM 호출 0). 단위 테스트 460 → **523 통과 / skipped 0 / 회귀 0**. assets/ **14/14 cover** (12건 정상 회귀 + 2건 거부 동작 회귀). E1 진단·완료 ship 은 PDF 보유 다른 컴퓨터에서 진입. S1 진척률 35→**65%** (D4 ship 누적).
...
### 4.2 S1 — 골든셋 v1 + 실 query 로그 (목표 1주, 약 **65% 진척**)
...
> 2026-05-07 D3 진입 — 8 commit push (`a5cfc2a`→`f1c87b4`) + **S1 D3·D4 ship 추가**. baseline 회복 + E1 sprint plan 본문 ship + S0 D4 자동 POST 제거 (+ P2 follow-up 3건) + S1 D1 잔여 (`auto_goldenset.py` v2 + 30 테스트) + **E2 1·2·3·4·5차 ship** (`assets/public/` 8건 git 추적, 5단계 우선순위 5개 형식 자동 인식, `test_hwp_heading.py`/실 fixture 테스트 클래스 신규, **negative path 회귀 + frontend `break-words` 가독성 보강**) + **E1 1차 ship 일부 즉시 진입 — 인제스트 진행 표시 3대 증상 일괄 fix** + **S1 D2 ship — 자동 골든셋 100+ 확장 + v1 통합 + retrieval fallback chain v1 우선** + **S1 D3 ship — 실 query 로그 시각화 대시보드** + **S1 D4 ship — answer_feedback 통합 분석 대시보드** (`/admin/feedback` 페이지 + `GET /admin/feedback/stats` endpoint 신규, `classify_comment` 룰 4 카테고리 source>search>answer>other 우선순위 LLM 호출 0, 일별 sparkline + 코멘트 카테고리 분포 + 최근 10건 코멘트, 13 단위 테스트). 단위 테스트 460→**523** / skipped 0 / 회귀 0. **assets/ 14/14 cover** (12 정상 + 2 거부 동작 회귀). S1 진척률 15→**65%**. E1 잔여 진단·구현은 PDF 보유 다른 컴퓨터에서 진입.
