...
Deno.test("JPEG APP1 을 읽는다 — APP0 뒤에 있어도 찾는다", () => {
  ...
  const bytes = new Uint8Array([
    0xff, 0xd8,
    0xff, 0xe0, 0x00, 0x04, 0x00, 0x00, // APP0 더미
    0xff, 0xe1, ((app1.length + 2) >> 8) & 0xff, (app1.length + 2) & 0xff, ...app1,
...
Deno.test("리틀엔디언 TIFF 도 읽는다", () => {
  const tiff = [
    ...
    0x08, 0x00, 0x00, 0x00,
    0x01, 0x00,
    0x12, 0x01, 0x03, 0x00, 0x01, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, // Orientation=8
  ];
...
Deno.test("범위 밖 방향값은 무시한다", () => {
  const mk = (v: number) => {
    const tiff = [
      ...
      0x01, 0x12, 0x00, 0x03, 0x00, 0x00, 0x00, 0x01, (v >> 8) & 0xff, v & 0xff, 0x00, 0x00,
    ];
    const app1 = [0x45, 0x78, 0x69, 0x66, 0x00, 0x00, ...tiff];
